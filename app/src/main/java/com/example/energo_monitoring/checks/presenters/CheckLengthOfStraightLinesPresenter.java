...
import com.example.energo_monitoring.checks.adapters.FlowTransducerPhotoAdapter;
import com.example.energo_monitoring.checks.viewmodel.CheckLengthViewModel;
...
public class CheckLengthOfStraightLinesPresenter {
    ...
    private final Step5_CheckLengthsFragment fragment;
    ...
    public CheckLengthOfStraightLinesPresenter(Step5_CheckLengthsFragment fragment,
                                               ClientDataBundle dataBundle, int dataId) {
        this.fragment = fragment;
        devices = new ArrayList<>(dataBundle.getDeviceFlowTransducers());

        photosAdapter = new FlowTransducerPhotoAdapter(fragment.requireContext(), this);
        ...
        takePhotoLauncher = fragment.registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        try {
                            Bitmap photo = MediaStore.Images.Media.getBitmap(fragment.requireActivity().getContentResolver(),
                                    lastCreatedPath);
    ...
    public void onPhotoCardClicked(FlowTransducerPhotoAdapter.PhotoViewHolder vh){
        if(vh.getLayoutPosition() == photos.size() - 1) {
            lastCreatedPath = LoadImageManager.getPhotoUri(fragment.requireContext());
            LoadImageManager.takePhoto(fragment.requireContext(), takePhotoLauncher, lastCreatedPath);
        }
    }
