package com.example.energo_monitoring.checks.ui.presenters.utilities
...
class LoadImageManager(
    ...
    companion object {
        ...
        @JvmStatic
        fun getPhotoUri(context: Context, saveRealPath: (String) -> Unit): Uri {
            var file: File? = null
            try {
                file = File.createTempFile("photo", ".png", context.filesDir)
        ...
        fun getBase64FromPath(context: Context, paths: String): String {
            ...
            if (result.length > 0) result.substring(0, result.length - 1)
