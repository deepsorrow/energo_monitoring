...
fun ChecksContent(
    ...
    val requestSavedClientsInfo = {
        showProgressBar = true
        viewModel.requestSavedAndOnlineInfo(context) {
    ...
    val syncInfo = { dataId: Int ->
        viewModel.loadInfo(
            ...
            onComplete = { cloudData ->
                ...
                if (cloudData != null && (localData == null || localData.otherInfo!!.dataId == 0 || cloudData.version > localData.otherInfo!!.localVersion)) {
                        loadingText = "Загрузка данных с облака..."
                        viewModel.saveDataFromCloud(context, dataId, cloudData)
                        requestSavedClientsInfo()
                    ...
                    viewModel.sendDataToCloud(
                        ...
                        onError = {
                            Toast.makeText(context, "Не удалось отправить данные! Ошибка: $it", Toast.LENGTH_LONG).show()
                            showProgressBar = false
                        })
                } else {
                    Toast.makeText(context, "Не удалось синхронизировать! Код ошибки: 561", Toast.LENGTH_LONG).show()
                }
            },
            onError = {
                Toast.makeText(context, "Не удалось отправить данные! Ошибка: $it", Toast.LENGTH_LONG).show()
    ...
    OnLifecycleEvent { owner, event ->
        when (event) {
            Lifecycle.Event.ON_RESUME -> { requestSavedClientsInfo() }
            else                      -> {  }
        }
    }
                ...
                content = {
                    if (viewModel.clients.isEmpty()) {
                        NoDataPlaceholder(
                            ...
                            iconRes = R.drawable.ic_baseline_hourglass_empty_24
                        )
                    } else {
                        LazyColumn {
                                    ...
                                    onActionClick = { selectedAction: DropDownClientActions, clientInfo: ClientInfo ->
                                        when (selectedAction) {
                                            ...
                                            DropDownClientActions.REMOVE_GLOBALLY -> {
                                                viewModel.onActionClicked(selectedAction, context, clientInfo)
                                                requestSavedClientsInfo()
                                            }
                                        }
                                    },
